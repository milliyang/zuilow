"""
Bull5dRandom: 测试用策略。通过 DMS 拉取当前日期前历史，做简单回测筛选后随机选 5 只、随机权重。
...
class Bull5dRandom(Strategy):
    """
    通过 DMS 获取历史、简单回测筛选后随机选 5 只、随机权重。无需配置参数。
    """

    def __init__(self, **kwargs):
        super().__init__(name="Bull5dRandom")
    ...
    def get_rebalance_output(self) -> dict | None:
        ...
        try:
            ...
            logger.warning("Bull5dRandom: DataSourceManager not available")
        ...
        logger.info("Bull5dRandom: symbols count %d, selected %d", len(symbols), len(sel_symbols))

        for symbol in sel_symbols:
            try:
                logger.info(f"Bull5dRandom: get_history {symbol} {start_date} {end_date}")
                ...
                logger.debug("Bull5dRandom: get_history %s failed: %s", symbol, e)
    ...
    def _random_allocation(self, pool: list[str]) -> dict | None:
        ...
        logger.info("Bull5dRandom: get_rebalance_output() %s", weights)
        return out
