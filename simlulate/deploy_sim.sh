#!/bin/bash
# Simulation stack: start / stop / restart / logs / clean.
...
_init() {
    ...
    cp env.example .env
    _write_repo_root
    echo "Created .env from env.example (REPO_ROOT=$REPO_ROOT). Edit docker/.env to set DMS_BASE_URL etc., then run $0 up."
}
...
_up() {
    ...
    echo "  Stime (set/advance):     http://localhost:${STIME_PORT:-11185}"
    echo "  ZuiLow:                  http://localhost:${ZUILOW_PORT:-11180}"
    echo "  PPT:                     http://localhost:${PPT_PORT:-11182}"
    echo ""
    echo "DMS is external. Set DMS_BASE_URL in docker/.env if needed."
    echo "View logs: $0 logs [stime|zuilow|ppt]"
    echo "Run dirs (log/DB): $SCRIPT_DIR/run/{zuilow,ppt,stime}"
    echo ""
}
...
_usage() {
    ...
    echo "  up       Start stime, zuilow, ppt (build if needed)."
